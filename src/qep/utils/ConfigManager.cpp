...
bool ConfigManager::printSolverDetails() const
{
    return data_->value("logging", nlohmann::json::object()).value("print_solver_details", false);
}
...
AppConfig AppConfig::fromConfig(const ConfigManager &cfg) {
    ...
    c.print_solver_details  = log.value("print_solver_details", false);
