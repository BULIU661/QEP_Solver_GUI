...
推荐使用 **VS Code + CMake Tools 扩展**（已配置好编译器环境）：

1. VS Code 打开项目根目录
2. `Ctrl+Shift+P` → **CMake: Configure** → 选择 Release
3. **CMake: Build**

命令行构建需在 **Developer Command Prompt for VS** 或指定编译工具：
...
```bash
cmake -B build -G Ninja -DQEP_BUILD_GUI=OFF    # 仅构建 CLI
cmake -B build -G Ninja -DQEP_BUILD_CLI=OFF    # 仅构建 GUI
cmake -B build -G Ninja -DQEP_USE_MKL=OFF      # 不使用 MKL
```

若 Qt 未安装在默认路径，需显式指定：

```bash
cmake -B build -G Ninja -DCMAKE_PREFIX_PATH=C:/Qt/5.15.2/msvc2019_64
```

构建产物：
- `bin/qep.exe` — CLI 可执行文件
- `bin/qep_gui.exe` — GUI 可执行文件
- `lib/QEP.lib` — 核心静态库
